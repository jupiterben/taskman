...
const ListContent: FC<{ data: API.TaskStatusMsg }> = ({ data }) => {
    ...
    return (
        <div className={styles.listContent}>
            ...
            <div className={styles.listContentItem}>
                <p>{stateData.desc}</p>
                <Progress percent={progress} status={status} strokeWidth={6} style={{ width: 220 }} />
            </div>
        </div>
    )
};
...
function renderTask(task: API.TaskStatusMsg) {
    ...
    return (
        <List.Item>
            <List.Item.Meta
                title={task.meta.uuid}
                description={customData.maxFile}
            />
...
const JobStatus: React.FC<{ data: API.JobStatus }> = (props) => {
    const { data } = props;
    const runningNum = data.tasks.filter(task => task.stateData.state == TaskStateEnum.Running).length;
    const inqueueNum = data.tasks.filter(task => task.stateData.state == TaskStateEnum.Start).length;
